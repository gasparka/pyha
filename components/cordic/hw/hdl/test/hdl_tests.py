...
@cocotb.coroutine
def run_dut(dut, x1, x2, x3, data_width):
    cocotb.fork(Clock(dut.clk, 5000).start())
    yield reset(dut)
    ...
    x3 = np.copy(x3)
    x3 = to_sfixed_real(x3, data_width)

    y1 = []
    y2 = []
    y3 = []
    print(type(dut))
    # for xi1, xi2, xi3 in zip(x1, x2, x3):
    #     dut.x1 = xi1.astype(int)
    #     dut.x2 = xi2.astype(int)
    #     dut.x3 = xi3.astype(int)
    for _ in range(32):
        dut.x1 = x1.astype(int)
        dut.x2 = x2.astype(int)
        dut.x3 = x3.astype(int)
        # FIXME: NOTICE, need to have both yields to match simulation.
        yield RisingEdge(dut.clk)
        yield FallingEdge(dut.clk)
        y1.append(float(dut.y1.value.signed_integer))
        y2.append(float(dut.y2.value.signed_integer))
        y3.append(float(dut.y3.value.signed_integer))

    y1 = to_real(np.array(y1), data_width)
    y2 = to_real(np.array(y2), data_width)
    y3 = to_real(np.array(y3), data_width)

    # plt.plot(y1)
    # plt.plot(y2)
    # plt.show()
    raise ReturnValue((y1, y2, y3))
...
def test_main(dut):
    ...
    hdl_out = yield run_dut(dut, 0.2, -0.01, np.pi, 20)
    print(hdl_out[0])
    print(hdl_out[1])
    print(hdl_out[2])
