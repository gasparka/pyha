...
from pyha.conversion.conversion_types import escape_reserved_vhdl, VHDLModule, init_conversion_type, VHDLEnum, VHDLList
...
class DefNodeConv(NodeConv):
    ...
    def build_arguments(self):
        ...
        args = [init_conversion_type(name, val, val) for name, val in zip(argnames, argvals)]
        ...
        if self.data.last_return is not None:
            if isinstance(self.data.last_return, tuple):  # multiple returns
                rets = [init_conversion_type(f'ret_{i}', val, val)
                        for i, val in enumerate(get_iterable(self.data.last_return))]
            else:
                rets = [init_conversion_type(f'ret_{0}', self.data.last_return, self.data.last_return)]
    ...
    def build_variables(self):
        ...
        variables = [init_conversion_type(name, val, val)
    ...
    def build_typedefs(self):
        ...
        for function in self.value:
            if not isinstance(function, DefNodeConv):
                continue
            variables = [init_conversion_type(name, val, val) for name, val in function.data.locals.items()]
...
class SubmoduleDeepcopy:
    ...
    def apply(red_node):
        ...
        for x in nodes:
            target_type = init_conversion_type('-', super_getattr(convert_obj, str(x.target)))
    ...
    def transform_prefix(red_node):
        ...
        def modify_call(red_node):
            ...
            if prefix.dumps() not in ['self', 'self.next']:
                ...
                var = init_conversion_type('-', var, var)
